...
test('enters a terminal timing-error state for an invalid confirmation deadline', () => {
  const reservation = {
    due_at: '2026-07-26 12:00:00',
    confirmation_due_at: 'not-a-time',
  };
  const state = resolveReservationTimeState(reservation, Date.UTC(2026, 6, 26, 12, 2, 0));
  assert.equal(state.phase, 'timing-error');
  if (state.phase === 'timing-error') assert.match(state.error, /\S/);
});
